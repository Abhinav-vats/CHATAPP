...
app.config(['$stateProvider','$urlRouterProvider',function($stateProvider,$urlRouterProvider){
    ...
    .state('loggedin',{
        ...
        views:{
            'body':{
                templateUrl:'/views/teacher_chat.html',
                controller:'myController'
            }
        }
    })
...
//        }
        ...
        div.innerHTML='<div class="popup-box popup-box-on chat-popup" id="'+chat_friend+'01">\
                        ...
                        <div class="popup-head-left pull-left"><span class="fa fa-user"></span>'+chat_friend+'</div>\
...
//        $compile(body)($scope);
    ...
    $scope.submitRepo = function(){
        $http({method: 'POST', url:'http://'+url+'/teacher-login/report', data:$scope.repo})
        .success(function(data){
            console.log(data)
        })
        .error(function(err){
...
app.controller('registerController',['$scope','encrypt','$http','$state',function($scope,encrypt,$http,$state){
    ...
    $scope.otpCheck = function(){
        $http({method: 'POST', url:'http://'+url+'/otp', data:$scope.Otp})
        .success(function(data){
            console.log(data)
        })
        .error(function(err){
            window.alert("Your Email Id is Blocked by Some teacher");
            console.log("ERR: ",err);
        });
    }
